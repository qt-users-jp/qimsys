...
void InputContext::Private::setFocusObject(QObject *o)
{
    ...
    qulonglong currentWidget = 0;
    widgetAndWindowFor(focusObject, &currentWidget, 0);
    ...
    if (o) {
        ...
        if (!currentItem.to.isEmpty()) {
            sendCommitString(currentItem.to.join(""), currentWidget);
            currentItem = QimsysPreeditItem();
            if (preeditManager)
                preeditManager->setItem(currentItem);
        }
        if (applicationManager) {
            if (applicationManager->widget() == currentWidget) {
...
{
    if (!focusObject || (qulonglong)focusObject != target) return;
...
{
    qimsysDebugIn();
    if (preeditManager && focusObject && applicationManager && applicationManager->composing()) {
        QInputMethodQueryEvent query(queries);
        QCoreApplication::sendEvent(focusObject, &query);
        if (queries & Qt::ImEnabled) {
            bool enabled = query.value(Qt::ImEnabled).toBool();
            if (!enabled) {
                setFocusObject(0);
            }
        ...
        if (queries & Qt::ImCursorRectangle) {
            ...
            QPoint topLeft;
            if (qobject_cast<QWidget *>(focusObject)) {
                topLeft = qobject_cast<QWidget *>(focusObject)->mapToGlobal(QPoint(0, 0));
//            } else if (qobject_cast<QQuickItem *>(focusObject)) {
                // TODO
//                topLeft = qobject_cast<QQuickItem *>(focusObject)->mapToGlobal(QPoint(0, 0));
            }
            r.translate(topLeft);
