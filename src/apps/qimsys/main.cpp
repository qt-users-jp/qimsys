...
{
    int ret = 0;
#ifdef QIMSYS_PLATFORM_LINUX
    if (qgetenv("XMODIFIERS") == QByteArray("@im=qimsys")) {
    ...
    if (qgetenv("QT_IM_MODULE") == QByteArray("qimsys")) {
        qputenv("QT_IM_MODULE", QByteArray());
    }
#endif
    if (QimsysCommandlineArgs::instance()->parse(argc, argv)) {
        Application app(argc, argv);
#ifdef QIMSYS_PLATFORM_LINUX
        qputenv("XMODIFIERS", QByteArray("@im=qimsys"));
